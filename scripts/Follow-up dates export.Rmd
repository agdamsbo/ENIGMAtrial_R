...
# End date is 90 days after first date
end.date<-as.Date(min(df$start))+90
...
df <- df |> 
  ...
  mutate(start=format(as.POSIXct(start), format = "%A d. %d/%m kl %H:%M")) |> 
  select(start, id, name) 
```
...
```{r}
knitr::kable(apply(df,MARGIN = 1,FUN = function(i){
