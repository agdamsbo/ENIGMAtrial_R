...
```{r}
tbl_folder<-"/Users/andreas/Documents/Læge/Forskning/PhD/ENIGMA/RBANS/RBANS_index_tables"
...
```{r}
interval_extention<-function(x,coln=1,df=TRUE){
  ...
  if (df==TRUE&class(x)!="data.frame"|class(x)=="data.frame"&ncol(x)==1){stop("check that input is truly data frame with at least two rows or set df=FALSE")}
  if (df==TRUE){
...
interval_extention(test[1:2],df=T)
```
